defmodule EvalioAppWeb.ReminderTagMenu do
  ...
  def render(assigns) do
    ...
    <div class="relative">
      <button
        type="button"
        class={"w-[40px] h-[25px] #{tag_color_class(@reminder.tag)} text-white text-xs flex items-center justify-center rounded-full cursor-pointer hover:#{tag_color_class(@reminder.tag)} transition-all outline-none"}
        phx-click={JS.toggle(to: "#dropdown-menu-#{@reminder.id}")}
      >
      </button>
